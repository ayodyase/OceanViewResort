...
public class RoomAvailabilityDao {
    ...
    public List<RoomAvailability> findAll() {
        String sql = "SELECT id, room_number, availability_date, availability_status, notes " +
                "FROM room_availability ORDER BY availability_date DESC, id DESC";
    ...
    public void create(RoomAvailability availability) {
        String sql = "INSERT INTO room_availability (room_number, availability_date, availability_status, notes) " +
                "VALUES (?, ?, ?, ?)";
        jdbcTemplate.update(sql,
                trim(availability.getRoomNumber()),
                trim(availability.getDate()),
    ...
    public void update(RoomAvailability availability) {
        String sql = "UPDATE room_availability SET room_number = ?, availability_date = ?, availability_status = ?, " +
                "notes = ? WHERE id = ?";
        jdbcTemplate.update(sql,
                trim(availability.getRoomNumber()),
                trim(availability.getDate()),
    ...
    private RowMapper<RoomAvailability> availabilityRowMapper() {
        return new RowMapper<RoomAvailability>() {
            ...
            public RoomAvailability mapRow(ResultSet rs, int rowNum) throws SQLException {
                ...
                availability.setDate(rs.getString("availability_date"));
